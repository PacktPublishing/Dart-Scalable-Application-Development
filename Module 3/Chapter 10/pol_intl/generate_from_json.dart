...
main(List<String> args) {
  ...
  parser.addFlag("suppresswarnings", defaultsTo: false,
      callback: (x) => suppressWarnings = x);
  parser.addOption("outputdir", defaultsTo: '.',
      callback: (x) => targetDir = x);
  parser.addOption("generatedfileprefix", defaultsTo: '',
  ...
  if (dartFiles.length == 0 || jsonFiles.length == 0) {
    print('Usage: generate_from_json [outputdir=<dir>]'
        ' [generatedfileprefix=<prefix>] file1.dart file2.dart ...'
        ' translation1.json translation2.json ...');
    exit(0);
  }

  // We're reparsing the original files to find the corresponding messages,
